...
function calculateELO() {
  // 주종 입력
  var docplayer = SpreadsheetApp.openById(sheetAddress).getSheetByName('PlayersData');
  ...
  var docplayerInit = SpreadsheetApp.openById(sheetAddress).getSheetByName('초기티어');
  ...
  for (i=0; i<numRowPlayerInit-1;i++) {
    ...
    PlayersElo.set(playerInit, changeElo_init(playerInitTier, playerInitTierChanged)); // 2023.4.2 이전의 티어로 초기화를 함.
  ...
  for (jj=0; jj<numRow-1; jj++) {
    ...
    record = [[PlayersElo.get(winner),newELO[0]-PlayersElo.get(winner),newELO[0], PlayersElo.get(loser),newELO[1]-PlayersElo.get(loser),newELO[1],gameRefPoint, gameLeagueWeight, gameTierWeight, gameSetWeight, time_weight, newELO[2], newELO[3], PlayersElo.get(winner)-PlayersElo.get(loser) ]];
  ...
  var docELOResultAll = SpreadsheetApp.openById(sheetAddress).getSheetByName('모든 선수 데이터');
  var cnt = 1
...
function weightLeague(league) {
  if (league.includes("프로리그")|| league.includes("클랜친선경기")  || league.includes("KPL")  || league.includes("WPL")|| league.includes("3050 프로리그")  || league.includes("사일배 클랜리그")|| league.includes("TPL")  || league.includes("WE CLAN LEAGUE") || league.includes("AHCL") || league.includes("&클랜 ESL")  || league.includes("WHITE 클랜팀리그") ) return [weight_ClanTeam, "team"]; // 클랜 팀리그
  else if (league.includes("KALEAGUE") || league.includes("하믹 청정수리그 S3") || league.includes("청정수팀리그")  || league.includes("홍토스배 친선하믹클랜전") ) return [weight_EventTeam, "team"] ; // 하믹 이벤트 팀리그
  else if (league.includes("청정수리그") || league.includes("포워드배 오픈컵")  || league.includes("하믹 연승전") || league.includes("홍토스배 CEL2 야간종족최강전") || league.includes("사일배 당일치기 토너먼트") || league.includes("끝장전")) return [weight_EventIndi, "indi"] ; // 이벤트 개인리그 
  else if (league.includes("team") || league.includes("Team") || league.includes("TEAM")) return [weight_STL, "team"] ; // hamic team leauge
  else if (league.includes("HEL")||league.includes("AEL")||league.includes("MEL")||league.includes("IEL")||league.includes("CEL")||league.includes("HST")) return [weight_SILHST, "indi"] ; // 
...
function updateStat(stat, league, win) {
  if (league.includes("프로리그") || league.includes("KALEAGUE")  || league.includes("하믹 청정수리그 S3") || league.includes("청정수팀리그")  || league.includes("클랜친선경기") || league.includes("KPL")  || league.includes("WPL")|| league.includes("3050 프로리그")  || league.includes("사일배 클랜리그")|| league.includes("TPL")  || league.includes("WE CLAN LEAGUE")|| league.includes("AHCL")  || league.includes("WHITE 클랜팀리그") || league.includes("홍토스배 친선하믹클랜전") ) {
    if (win == true ) {stat.Owin = stat.Owin + 1;}
    else {stat.Olose = stat.Olose + 1;}
  }
  else if (league.includes("청정수리그") || league.includes("포워드배 오픈컵")  || league.includes("하믹 연승전")|| league.includes("청정수팀리그")|| league.includes("홍토스배 CEL2 야간종족최강전") || league.includes("사일배 당일치기 토너먼트") ||  league.includes("클랜스타리그")|| league.includes("끝장전") ) {
    if (win == true ) {stat.Owin = stat.Owin + 1;}
    else {stat.Olose = stat.Olose + 1;}
  }
  else if (league.includes("team") || league.includes("Team") || league.includes("TEAM")  ) {
